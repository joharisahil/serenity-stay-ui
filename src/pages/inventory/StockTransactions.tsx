...
const StockTransactions = () => {
  ...
  const filtered = stockTransactions.filter(txn => {
    return (typeFilter === 'all' || txn.type === typeFilter) && (refFilter === 'all' || txn.referenceType === refFilter);
  });
    ...
    <AppLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Complete audit trail of all stock movements</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th className="text-center px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Reference</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Qty</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Balance</th>
                    <th className="text-left px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Notes</th>
                    <th className="text-left px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">By</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {filtered.map(txn => (
                    <tr key={txn.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-5 py-3 font-mono text-xs">{txn.itemSku}</td>
                      <td className="px-5 py-3 font-medium">{txn.itemName}</td>
                      <td className="px-5 py-3 text-center">
                        <Badge variant="outline" className={typeStyles[txn.type]}>{txn.type}</Badge>
                      </td>
                      <td className="px-5 py-3 text-center">
                        <Badge variant="outline" className={`${refStyles[txn.referenceType]} text-xs`}>{txn.referenceType}</Badge>
                      </td>
                      <td className={`px-5 py-3 text-right font-semibold ${txn.type === 'IN' ? 'text-success' : 'text-destructive'}`}>
                        {txn.type === 'IN' ? '+' : '-'}{Math.abs(txn.quantity)}
                      </td>
                      <td className="px-5 py-3 text-right">{txn.balanceAfter}</td>
                      <td className="px-5 py-3 text-xs text-muted-foreground max-w-[200px] truncate">{txn.notes}</td>
                      <td className="px-5 py-3 text-xs text-muted-foreground">{txn.createdBy}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
